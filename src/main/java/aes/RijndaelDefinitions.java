...
//    private static final int S_BOX_LENGTH = 16;
//    private final int[] substitutionBox = { 0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
...
//                                            0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16};
    ...
    byte getSubstitutedByte(byte value) {
        return (byte) substitutionBox[value & 0xff];
    }

    byte multiplyBy2(byte value) {
        return multiplyBy2LookupTable[value & 0xff];
    }

    byte multiplyBy3(byte value) {
        return multiplyBy3LookupTable[value & 0xff];
    }

    byte getInverse(byte value) {
        return (byte) multiplicativeInverse[value & 0xff];
    }
    ...
    private byte multiplicationGalois256(byte firstByte, byte secondByte) {
        ...
        for (int counter = 0; counter < 8; counter++) {
            ...
            if (isCarrySet) {
                firstByte ^= 0b1_0001_1011; /* x^8 + x^4 + x^3 + x + 1 */
            }
    ...
    private byte circularLeftShift(byte value, int shiftCount) {
        return (byte) ((value << shiftCount) | (value >> (Byte.SIZE - shiftCount)));
    }
}
